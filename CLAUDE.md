...
## 9. Excel & CSV Conversion Feature

> **Purpose:** Extend SpecStream beyond PDF-only input by supporting Excel (`.xlsx`, `.xls`) and CSV/TSV (`.csv`, `.tsv`) files — converting them into structured Markdown and JSON output that integrates with the existing viewer, tree outline, and JSON export panels.

### Technical Stack

| Layer | Tool | Role |
|---|---|---|
| Client-side parsing | **SheetJS (`xlsx`)** | Reads Excel and CSV/TSV files entirely in the browser. Produces sheet-level JSON arrays and cell metadata (types, formatting, merged ranges). No server round-trip required. |
| CLI / pipeline fallback | **csvkit** | Command-line toolkit (`csvjson`, `csvlook`, `in2csv`) for server-side or batch conversion. `in2csv` normalizes `.xlsx`/`.xls` → CSV; `csvjson` emits JSON; `csvlook` emits Markdown-style ASCII tables. |

### SheetJS Integration (Browser)

Import via CDN in `<head>`:
```html
<script src="https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js"></script>
```

Core conversion flow (JavaScript):
```javascript
...
workbook.SheetNames.forEach((name, sheetIndex) => {
  ...
  // → JSON (array of row-objects, headers from first row)
  const json = XLSX.utils.sheet_to_json(sheet, { defval: '' });

  // → Markdown table via intermediate HTML
  const html = XLSX.utils.sheet_to_html(sheet);
  // or build Markdown directly from json (see Markdown generation below)
});
```

#### Markdown Table Generation

Markdown tables are built from the JSON row-objects so that column alignment and escaping are controlled precisely:

```javascript
function sheetToMarkdown(json) {
  if (!json.length) return '_Empty sheet_';
...
Each sheet produces a block that mirrors the existing SpecStream JSON element schema:
...
```json
{
  ...
  "row_count": 42,
  "col_count": 7,
  "headers": ["SKU", "Description", "Qty", "Unit Price", "Total", "Tax", "Notes"],
  "content": [
    { "SKU": "A-001", "Description": "Widget", "Qty": 100, "Unit Price": 2.50, "Total": 250.00, "Tax": 25.00, "Notes": "" }
  ]
}
```
...
| Field | Description |
|---|---|
| `type` | Always `"table"` — reuses the existing table block type so the tree badges, bbox stroke (`--color-table` / `#3b82f6`), and fill (`--bg-overlay-table`) apply automatically |
| `source` | `"spreadsheet"` — distinguishes from PDF-extracted tables |
| `sheet_name` | Original worksheet tab name |
| `sheet_index` | 0-based position within the workbook |
| `headers` | Column names (first-row values) |
| `content` | Array of row-objects keyed by header name |

#### Multi-Sheet Handling

Workbooks with multiple sheets render as **top-level tree nodes** in the sidebar outline. Each sheet becomes a collapsible parent node with its table rows as children:

```
▼ Sheet1  (table · 42 rows × 7 cols)
    • Row 1 …
    • Row 2 …
▼ Sheet2  (table · 18 rows × 4 cols)
    • Row 1 …
```

- Sheet nodes use the `table` badge style (blue, `#3b82f6` / `#60a5fa`)
- A secondary meta-pill shows `Sheets: N` alongside the existing `Pages:` / `Blocks:` / `Time:` pills in the viewer controls bar

### csvkit Integration (CLI / Server-Side)

For batch or server-side pipelines, csvkit provides the same conversions without a browser:
...
# CSV → Markdown-style ASCII table (useful for LLM prompts)
csvlook output.csv > output.md

# Specific sheet from a multi-sheet workbook
in2csv --sheet "Q4 Revenue" spreadsheet.xlsx | csvjson > q4.json
```

csvkit is a Python package (`pip install csvkit`) with no binary dependencies.

### UI Additions

#### Header Toolbar

The existing file picker (`<input type="file">`) expands its `accept` attribute:

```html
<input type="file" accept=".pdf,.xlsx,.xls,.csv,.tsv" />
```

No new buttons are added. The process pipeline detects the file type from the extension / MIME type and routes to either the PDF parser or the SheetJS converter.

#### Status Bar Messages

| Event | Status dot class | Message |
|---|---|---|
| Excel/CSV selected | `.info` | `Spreadsheet loaded — {filename}` |
| Conversion complete | `.success` | `Converted {N} sheet(s) · {R} total rows · {T}ms` |
| Parse error | `.error` | `Failed to parse spreadsheet: {error}` |

#### Viewer Panel (Left)

Since spreadsheets have no visual page to render on a canvas, the viewer panel switches to a **rendered HTML table view** when the input is a spreadsheet:

- Background: `--bg-main` (unchanged)
- Table styling inherits from the existing design tokens:
  - Header row: `background-color: var(--bg-input)`, `color: var(--text-main)`, `font-weight: 600`, `font-family: var(--font-mono)`, `font-size: 0.75rem`
  - Body cells: `color: var(--text-secondary)`, `font-family: var(--font-mono)`, `font-size: 0.75rem`
  - Borders: `1px solid var(--border-color)`
  - Cell padding: `0.375rem 0.625rem`
  - Alternating rows: odd rows `background-color: var(--bg-card)`, even rows transparent
  - Hover row: `background-color: rgba(99, 102, 241, 0.08)` (subtle indigo tint, consistent with tree selection)
- Selected row (clicked): `background-color: rgba(99, 102, 241, 0.15)`, `border: 1px solid rgba(99, 102, 241, 0.3)` — identical to the tree-selected state
- Sheet tabs (when multi-sheet): rendered as a tab bar above the table, using `.btn-secondary` styling with the active tab using `.btn-primary` styling

#### JSON Export

The JSON export textarea and copy-to-clipboard behavior remain identical. The output is the full JSON array of sheet blocks described above, formatted with 2-space indentation and displayed in terminal green (`#34d399`) on `--bg-main`.
...
```
  ...
  ┌──────────┐  ┌──────────────────────────┐
  │  PDF     │  │  SheetJS (browser)       │
  │  Parser  │  │  or csvkit (CLI)         │
  └────┬─────┘  └────────────┬─────────────┘
       │                     │
       ▼                     ▼
  ┌──────────────────────────────────────────┐
  │  Unified JSON Schema                     │
  │  (type: heading | table | spec | …)      │
  └────────────────────┬─────────────────────┘
                       │
          ┌────────────┼────────────┐
          ▼            ▼            ▼
     Tree Outline   Viewer      JSON Export
     (sidebar)      (canvas     (textarea)
                    or table)
```
